<style>
	.watermark{
		background:url("https://lh3.googleusercontent.com/proxy/pcl1Wg6UDcZS8Qr6HlFZXiWcSUmI1wRQGjJWoPfFK9VrPRPFy67CFJ8WPacK18Ci1IzWSuF5_9iTOtgFDrErMeyx1QyJAXu3TqexmBnbxJI3Fh_3590YGMC4ho20cZDAxiYCXA") center center no-repeat;
		opacity: 0.3;
		position: absolute;
		width: 100%;
		height: 100%;
	}
</style>

<div class="main-content">
	<div class="watermark"></div>
	<div class="container-fluid">
		<div class="row">
			<div class="col-12" style="padding:10px;border: 2px solid darkgoldenrod;">
				<div class="row">
					<div class="col-2">
						<img src="data:image/png;base64," alt="">
					</div>
					<div class="col-9">
						<h2>TAGIHAN SERVICE CHARGE</h2>
						<p class="title-kwitansi">THE KINGS SHOPPING CENTER
							<br/>JL KEPATIHAN 11 12 # BANDUNG # INDONESIA
							<br/>+621223165037
							<br/>MANAGEMENT@THEKINGS.CO.ID # WWW.THEKING.ID
						</p>
					</div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;border-top: 2px solid darkgoldenrod;border-bottom: 2px solid darkgoldenrod;">
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tanggal</div>
					<div class="col-1">:</div>
					<div class="col-3">01 FEBRUARI 2020</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">No Bukti</div>
					<div class="col-1">:</div>
					<div class="col-3">000172/TSC/2020</div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;">
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tagihan Bulan</div>
					<div class="col-1">:</div>
					<div class="col-3">JANUARI 2020</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Nama Toko</div>
					<div class="col-1">:</div>
					<div class="col-3">CV. Surona Bella (Upnormal)</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Nama Pemilik</div>
					<div class="col-1">:</div>
					<div class="col-3">ALVIN BENJAMIN TJANDRA</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">No NPWP</div>
					<div class="col-1">:</div>
					<div class="col-3">-</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Kode Lokasi</div>
					<div class="col-1">:</div>
					<div class="col-3">GF-01A</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Luas (Mtr<sup>2</sup>)</div>
					<div class="col-1">:</div>
					<div class="col-3" style="text-align: right">138,67</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Harga per M<sup>2</sup></div>
					<div class="col-1">:</div>
					<div class="col-3">Rp <span style="float:right;">100.000</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tag Service Charge</div>
					<div class="col-1">:</div>
					<div class="col-3">Rp <span style="float:right;">13.887.000</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">PPN 10%</div>
					<div class="col-1">:</div>
					<div class="col-3">Rp <span style="float:right;">1.388.700</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Total Service Charge</div>
					<div class="col-1">:</div>
					<div class="col-3">Rp <span style="float:right;">15.275.700</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">B Administrasi</div>
					<div class="col-1">:</div>
					<div class="col-3">Rp <span style="float:right;">15.000</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Total Tagihan</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-top: 1px solid black;border-bottom: 1px solid black;">Rp <span style="float:right;">15.290.700</span></div>
				</div>
				<div class="row" style="margin-top: 20px;">
					<div class="col-12"><u>Penerimaan Pembayaran :</u></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tunai</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;">Rp <span style="float:right;"></span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Bank</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Giro / Cek / Nomer</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Transfer / Tgl JT</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Jumlah</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;">Rp <span style="float:right;"></span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Bandung, tanggal</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Atas Nama</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;border-top: 2px solid darkgoldenrod;border-bottom: 2px solid darkgoldenrod;">
				<h4 style="text-align: center">CARA PEMBAYARAN VIA BANK BCA MALALUI VIRTUAL ACCOUNT</h4>
				<h5 style="text-align: center">NOMOR ANDA 09593 920100</h5>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;">
				HARAP JANGAN SALAH KETIK <br/>
				MOHON DIPERIKSA KEMBALI JUMLAH TAGIHAN SEBELUM DI TRANSFER
				TANGGAL PEMBAYARAN PALING LAMBAT
				PADA TANGGAL 20 SETIAP BULANNYA
				REKENING A/N : PT DALEH MUKTI NO REKENING 008-780-2802
			</div>
		</div>
	</div>
</div>
