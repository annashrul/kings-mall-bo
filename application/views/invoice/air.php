<style>
	.watermark{
		background:url("https://lh3.googleusercontent.com/proxy/pcl1Wg6UDcZS8Qr6HlFZXiWcSUmI1wRQGjJWoPfFK9VrPRPFy67CFJ8WPacK18Ci1IzWSuF5_9iTOtgFDrErMeyx1QyJAXu3TqexmBnbxJI3Fh_3590YGMC4ho20cZDAxiYCXA") center center no-repeat;
		opacity: 0.3;
		position: absolute;
		width: 100%;
		height: 100%;
	}
</style>
<div class="main-content">
	<div class="watermark"></div>

	<div class="container-fluid">
		<div class="row">
			<div class="col-12" style="padding:10px;border: 2px solid darkgoldenrod;">
				<div class="row">
					<div class="col-2">
						<img src="data:image/png;base64," alt="">
					</div>
					<div class="col-9">
						<h2>TAGIHAN PEMAKAIAN AIR MINUM - PDAM</h2>
						<p class="title-kwitansi">THE KINGS SHOPPING CENTER
							<br/>JL KEPATIHAN 11 12 # BANDUNG # INDONESIA
							<br/>+621223165037
							<br/>MANAGEMENT@THEKINGS.CO.ID # WWW.THEKING.ID</p>
					</div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;border-top: 2px solid darkgoldenrod;border-bottom: 2px solid darkgoldenrod;">
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tanggal</div>
					<div class="col-1">:</div>
					<div class="col-3">01 FEBRUARI 2020</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">No Bukti</div>
					<div class="col-1">:</div>
					<div class="col-3">0002/TPAM/2020</div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;">
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tagihan Bulan</div>
					<div class="col-1">:</div>
					<div class="col-3">JANUARI 2020</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Nama Toko</div>
					<div class="col-1">:</div>
					<div class="col-3">CV. Surona Bella (Upnormal)</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Nama Pemilik</div>
					<div class="col-1">:</div>
					<div class="col-3">ALVIN BENJAMIN TJANDRA</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Kode Lokasi</div>
					<div class="col-1">:</div>
					<div class="col-3">GF-01A</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Meter Awal /Mtr<sup>3</sup></div>
					<div class="col-1">:</div>
					<div class="col-3" style="text-align: right">305,249</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Meter Akhir /Mtr<sup>3</sup></div>
					<div class="col-1">:</div>
					<div class="col-3" style="text-align: right;border-bottom: 1px solid black;">385,745</div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Pemakaian /Mtr<sup>3</sup></div>
					<div class="col-1">:</div>
					<div class="col-3" style="text-align: right">80,496</div>
				</div>

				<div class="row" style="margin-top: 20px;">
					<div class="col-12">PERINCIAN</div>
				</div>

				<div class="row" style="margin-top: 10px;">
					<div class="col-3">
						<div class="row">
							<div class="col-5">RP <span style="float:right;">4.600</span></div>
							<div class="col-1">x</div>
							<div class="col-5">RP <span style="float:right;">46.000</span></div>
						</div>
					</div>
					<div class="col-1"></div>
					<div class="col-3" style="">RP <span style="float:right;">71.000</span></div>
				</div>

				<div class="row" style="margin-top: 10px;">
					<div class="col-3">
						<div class="row">
							<div class="col-5">RP <span style="float:right;">7.200</span></div>
							<div class="col-1">x</div>
							<div class="col-5">RP <span style="float:right;">10.000</span></div>
						</div>
					</div>
					<div class="col-1"></div>
					<div class="col-3" style="">RP <span style="float:right;">71.000</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">
						<div class="row">
							<div class="col-5">RP <span style="float:right;">71.000</span></div>
							<div class="col-1">x</div>
							<div class="col-5">RP <span style="float:right;">10.000</span></div>
						</div>
					</div>
					<div class="col-1"></div>
					<div class="col-3" style="">RP <span style="float:right;">107.000</span></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">
						<div class="row">
							<div class="col-5">RP <span style="float:right;">14.400</span></div>
							<div class="col-1">x</div>
							<div class="col-5">RP <span style="float:right;">50.496</span></div>
						</div>
					</div>
					<div class="col-1"></div>
					<div class="col-3" style="">RP <span style="float:right;">727.142</span></div>
				</div>
				<div class="row" style="padding:10px;">
					<div class="col-7" style="border-top:1px solid black;"></div>
				</div>
				<div class="row">
					<div class="col-3">JUMLAH</div>
					<div class="col-1"></div>
					<div class="col-3" style="">RP <span style="float:right;">952.142</span></div>
				</div>
				<div class="row">
					<div class="col-3">AIR KOTOR 30%</div>
					<div class="col-1"></div>
					<div class="col-3" style="border-bottom: 1px solid black;">RP <span style="float:right;">285.643</span></div>
				</div>
				<div class="row">
					<div class="col-3">JUMLAH PENAGIHAN</div>
					<div class="col-1"></div>
					<div class="col-3" style="border-bottom: 1px solid black;">RP <span style="float:right;">1.237.785</span></div>
				</div>
				<div class="row">
					<div class="col-3">PPN 10%</div>
					<div class="col-1"></div>
					<div class="col-3" style="border-bottom: 1px solid black;">RP <span style="float:right;">123.779</span></div>
				</div>
				<div class="row">
					<div class="col-3">TOTAL PENAGIHAN</div>
					<div class="col-1"></div>
					<div class="col-3" style="border-bottom: 1px solid black;">RP <span style="float:right;">1.361.564</span></div>
				</div>
				<div class="row">
					<div class="col-3"></div>
					<div class="col-1"></div>
					<div class="col-3"><span style="float:right;">SUDAH TERMASUK PPN & PPH</span></div>
				</div>
				<div class="row" style="margin-top: 20px;">
					<div class="col-12"><u>Penerimaan Pembayaran :</u></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Tunai</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;">Rp <span style="float:right;"></span></div>
				</div>
				<div class="row">
					<div class="col-3"></div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Giro/Cek/Transfer</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;">Rp <span style="float:right;"></span></div>
				</div>
				<div class="row">
					<div class="col-3"></div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Bandung, tanggal</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
				<div class="row" style="margin-top: 10px;">
					<div class="col-3">Atas Nama</div>
					<div class="col-1">:</div>
					<div class="col-3" style="border-bottom: 1px dotted black;"></div>
				</div>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;border-top: 2px solid darkgoldenrod;border-bottom: 2px solid darkgoldenrod;">
				<h4 style="text-align: center">CARA PEMBAYARAN VIA BANK BCA MALALUI VIRTUAL ACCOUNT</h4>
			</div>
			<div class="col-12" style="margin-top:10px;padding:10px;">
				TANGGAL PEMBAYARAN PALING LAMBAT <br/>
				PADA TANGGAL 20 SETIAP BULANNYA
			</div>
		</div>
	</div>
</div>
